...
def draw_plot():
    ...
    slope, intercept, r_value, p_value, std_err = lng(df["Year"], df["CSIRO Adjusted Sea Level"])
    years = pd.Series(range(1880,2050))
    ax.plot(years, intercept + slope*years, 'r', label='first line of best fit')
    df2 = df.loc[df["Year"] >= 2000]
    slope2, intercept2, r_value2, p_value2, std_err2 = lng(df2["Year"], df2["CSIRO Adjusted Sea Level"])
